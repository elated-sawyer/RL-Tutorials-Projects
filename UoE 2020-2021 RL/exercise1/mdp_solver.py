        ...
        while True:
            ...
            for s in range(self.state_dim):
                ...
                for action in range(self.action_dim):
                    for s_1 in range(self.state_dim):
                        A[action] += self.mdp.P[s][action][s_1] * (self.mdp.R[s][action][s_1] + self.gamma * V[s_1])
                best_action_value = np.max(A)
                # Calculate the maximum of estimation error in this iteration
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for s in range(self.state_dim):
            ...
            # update the policy, choosing the best action all the time
            policy[s, best_action] = 1.0
        return policy
...
class PolicyIteration(MDPSolver):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s in range(self.state_dim):
                ...
                for action in range(self.action_dim):
                    for s_1 in range(self.state_dim):
                        v += policy[s][action] * self.mdp.P[s][action][s_1] * (
                                    self.mdp.R[s][action][s_1] + self.gamma * V[s_1])
                # Calculate the maximum of estimation error in this iteration
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        V = np.zeros([self.state_dim])
        ### PUT YOUR CODE HERE ###
        # The initial policy is random policy, choosing all available actions with the same probability.
        ...
        while True:
            ...
            policy_stable = True
            # check if the current policy is the best, and update policy
            for s in range(self.state_dim):
                # The best action under the current policy (the prob is maximum)
                ...
                for action in range(self.action_dim):
                    for s_1 in range(self.state_dim):
                        A[action] += self.mdp.P[s][action][s_1] * (self.mdp.R[s][action][s_1] + self.gamma * V[s_1])
                best_a = np.argmax(A)
                # check if the current action is the best, and update policy
                if current_a != best_a:
                    policy_stable = False
                policy[s] = np.eye(self.action_dim)[best_a]
            # check if the policy is stable
            if policy_stable:
                break
        return policy, V
